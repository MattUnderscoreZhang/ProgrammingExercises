...
int main(int argc, char *argv[])
{
  ...
  while (replaceable) {
      size_t nonterminal_loc = message.find("<");
      replaceable = (nonterminal_loc != string::npos);
      if (!replaceable) { break; }
      size_t nonterminal_size = message.find(">") - nonterminal_loc + 1;
      string nonterminal = message.substr(nonterminal_loc, nonterminal_size);
      const Production& production = grammar[nonterminal].getRandomProduction();
      string replacement = "";
      for (Production::const_iterator production_iter = production.begin(); production_iter < production.end(); ++production_iter) {
          replacement.append(*production_iter + " ");
      }
      replacement.pop_back();
      message.replace(nonterminal_loc, nonterminal_size, replacement);
  }
